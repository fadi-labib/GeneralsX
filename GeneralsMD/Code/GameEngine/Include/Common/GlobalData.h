...
public:
  ...
  Int   m_statsInterval;       ///< if so, how many is N?
	...
	Int m_wasmSlowmoFps;					///< -slowmofps N: logic fps under slow-mo (-slowmo/-bridge). 1..30; 30 = real time. Default 8.
	Bool m_wasmControlBridge;				///< -control: create the LLM-general ControlBridge and drain the /control channel each frame. Implied by -bridge. Off by default, so a normal game never pays for it.
	...
	AsciiString m_wasmBridgeSide;			///< Task 4.3: side (faction) of the skirmish AI the ControlBridge must bind to. With two skirmish AIs in the bridge match (bridge AI + fighting opponent), this makes the binding deterministic: tick() binds only to the build-list owner whose getSide() matches this. Empty => legacy "first build-list owner" behavior.
